...
{ param ($edition)
    ...
    {
        $Exe = "C:\temp\setup.exe"
        $XML = "C:\temp\Office365_FR_FR64.xml"
        $arguments = " /configure " + $XML
        start-process $Exe -args $arguments -Wait -NoNewWindow
    }
}
...
function downlaodGitHub
{
    ...
    Download2 -Source "https://raw.githubusercontent.com/SebasHilotech/OfficeUninstall/master/Office365_EN_EN_FR_FR64.xml" -Ouput "c:\temp\Office365_FR_FR64.xml"
    $file = New-Object System.Object
    $Name = "deploiementOffice365"
    $file | Add-Member -type NoteProperty -name NAME -Value $Name
    $file | Add-Member -type NoteProperty -name FULLPATH -Value "c:\temp\Office365_FR_FR64.xml"
    $files += $file
